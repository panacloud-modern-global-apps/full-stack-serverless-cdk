...
function extendLocalReporterToCatchPluginErrors({
  ...
  const addPluginNameToErrorMeta = (errorMeta, pluginName) => typeof errorMeta === `string` ? {
    context: {
  ...
  if (pluginName && (reporter === null || reporter === void 0 ? void 0 : reporter.setErrorMap)) {
    ...
    error = (errorMeta, error) => {
      const errorMetaWithPluginName = addPluginNameToErrorMeta(errorMeta, pluginName);
      reporter.error(extendErrorIdWithPluginName(pluginName, errorMetaWithPluginName), error);
    };

    panic = (errorMeta, error) => {
      const errorMetaWithPluginName = addPluginNameToErrorMeta(errorMeta, pluginName);
      reporter.panic(extendErrorIdWithPluginName(pluginName, errorMetaWithPluginName), error);
    };

    panicOnBuild = (errorMeta, error) => {
      const errorMetaWithPluginName = addPluginNameToErrorMeta(errorMeta, pluginName);
      reporter.panicOnBuild(extendErrorIdWithPluginName(pluginName, errorMetaWithPluginName), error);
    };
  }
