...
function createPacResolver(_str, _opts = {}) {
    ...
    opts.sandbox = Object.assign(Object.assign({}, createPacResolver.sandbox), opts.sandbox);
    // Construct the array of async function names to add `await` calls to.
    const names = Object.keys(opts.sandbox).filter(k => isAsyncFunction(opts.sandbox[k]));
    if (!opts.filename) {
